...
describe "Order Wave 2" do
  ...
  describe "Order.find_by_customer(customer_id)" do
    it "Can find the all the orders of the customer" do

      customer_id = 4
      id = 11
      fulfillment_status = :pending
      id_two = 86
      product = {"Papaya"=>46.45, "Butter"=>91.32, "Coriander Leaves"=>98.34}

      orders = Order.find_by_customer(customer_id)

      expect(orders[0].id).must_equal id
      expect(orders[1].products).must_equal product
      expect(orders[2].id).must_equal id_two
      expect(orders[2].fulfillment_status).must_equal fulfillment_status
