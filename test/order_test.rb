...
describe "Order Wave 2" do
  ...
  describe "Order.find_by_customer" do
    it "Returns array with matching instances" do
      order = Order.find_by_customer(1)

      expect(order).must_be_kind_of Array
      expect(order[0]).must_be_kind_of Order
    end

    it "Returns array length equal to number of instances that match" do
      order = Order.find_by_customer(20)

      expect(order.length).must_equal 7
    end
